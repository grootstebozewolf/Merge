    ...
    public class MethodBodyReader : IDisposable
    {
        ...
        public string GetBodyCode()
        {
            ...
            if (instructions != null)
            {
                ...
                {
                    instructions[i].OnSqlInScript += new ILInstruction.SqlInScriptEventHandler(MethodBodyReader_OnSqlInScript);
                    result += instructions[i].GetCode(instructions, i) + "\n";
                }
            }
            return result;
        }

        void MethodBodyReader_OnSqlInScript(object sender, ILInstruction.EventArgsSqlInScript e)
        {
            OnSqlInScript(this, new EventArgsSqlInScript { Instructions = e.Instructions, Operand = e.Operand, StackPointer = e.StackPointer});
        }
